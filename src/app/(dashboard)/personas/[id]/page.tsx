...
const estadoStyles: Record<Estado, { bg: string; dot: string }> = {
  Activo: { bg: "bg-green-50 dark:bg-green-500/20 text-green-700 dark:text-green-400", dot: "bg-green-500" },
  Visitante: { bg: "bg-[#f9c70c]/20 text-[#b8860b] dark:text-[#f9c70c]", dot: "bg-[#f9c70c]" },
  "En seguimiento": { bg: "bg-[#0ca6b2]/20 text-[#0ca6b2]", dot: "bg-[#0ca6b2]" },
  "En servicio": { bg: "bg-violet-100 dark:bg-violet-500/20 text-violet-800 dark:text-violet-300", dot: "bg-violet-500" },
  Inactivo: { bg: "bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400", dot: "bg-gray-300 dark:bg-gray-500" },
};
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data: row, error } = await supabase
        .from("personas")
        .select("id, cedula, nombre, telefono, email, fecha_nacimiento, edad, estado_civil, ocupacion, direccion, grupo_id, participacion_en_grupo, rol, estado, fecha_registro, ultimo_contacto, notas, created_at, grupos(nombre, imagen)")
        .eq("id", id)
        .single();
  ...
  if (notFound || !persona) {
    return (
      <div className="min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center gap-4 px-4">
        ...
        <Link href="/personas" className="text-[#0ca6b2] font-medium hover:underline">Volver a personas</Link>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-center gap-6">
            <div className="w-28 h-28 rounded-2xl bg-white dark:bg-[#2a2a2a] p-1.5 shadow-xl">
              <Avatar size={100} name={persona.nombre} variant="beam" colors={["#0ca6b2", "#18301d", "#e64b27", "#f9c70c", "#faddbf"]} />
            </div>
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-3 mb-2">
                <h1 className="text-3xl font-bold text-white">{persona.nombre}</h1>
                {persona.rol !== "Visitante" && (
                  <span className="px-3 py-1 rounded-full text-xs font-semibold bg-white/20 text-white">{persona.rol}</span>
                )}
                <span className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 ${estadoStyles[displayEstado].bg}`}>
                  <span className={`w-2 h-2 rounded-full ${estadoStyles[displayEstado].dot}`} />
                  {displayEstado}
                </span>
              </div>
              ...
              {displayEstado === "Visitante" && (
                <button 
                  ...
                  className="mt-2 text-[#f9c70c] text-sm flex items-center gap-2 hover:text-[#f9c70c]/80 transition group"
                >
              ...
              {displayEstado === "En seguimiento" && (
                <p className="mt-2 text-[#0ca6b2] text-sm flex items-center gap-2">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <div className="flex gap-3 items-center">
              ...
              {persona.telefono && (
                <a href={`tel:${persona.telefono}`} className="text-white/70 hover:text-white transition" title="Llamar">
                  <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" />
                  </svg>
                </a>
              )}
              <span className="text-white/50 cursor-not-allowed" title="Editar (próximamente)">
                <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                </svg>
              </span>
              <Link href="/personas" className="text-white/70 hover:text-white transition" title="Volver">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-100 dark:border-[#2a2a2a] p-6">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {persona.cedula && <InfoItem icon="id" color="blue" label="Cédula" value={persona.cedula} />}
              ...
              <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-100 dark:border-[#2a2a2a] p-6">
                ...
                <div className="space-y-3 mb-6">
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-[#333] bg-gray-50 dark:bg-[#252525] text-[#18301d] dark:text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#0ca6b2] transition resize-none"
                  />
                  {errorNota && (
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl bg-[#0ca6b2] text-white text-sm font-semibold hover:bg-[#0a8f99] disabled:opacity-50 disabled:cursor-not-allowed transition"
                  >
                ...
                {persona.notasHistorial.length === 0 ? (
                  ...
                  <ul className="space-y-4 border-t border-gray-100 dark:border-[#2a2a2a] pt-6">
                    {persona.notasHistorial.map((n) => (
                      <li
                        key={n.id}
                        className="rounded-xl border border-gray-100 dark:border-[#2a2a2a] bg-gray-50/80 dark:bg-[#252525]/80 p-4"
                      >
                        <p className="text-[#18301d] dark:text-white whitespace-pre-wrap leading-relaxed">{n.contenido}</p>
              ...
              <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-100 dark:border-[#2a2a2a] p-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-lg font-semibold text-[#18301d] dark:text-white">Historial de actividad</h2>
                </div>
                  ...
                  {persona.historial.map((item, i) => (
                    <div key={i} className="flex gap-4">
                      <div className="flex flex-col items-center">
                        <div className="w-3 h-3 rounded-full bg-[#0ca6b2]" />
                        {i < persona.historial.length - 1 && <div className="flex-1 w-0.5 bg-gray-200 dark:bg-[#333] mt-1" />}
                      </div>
            ...
            <div className="space-y-6">
              {/* Group */}
              <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-100 dark:border-[#2a2a2a] p-6">
                <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4">Grupo</h3>
                {persona.grupoId ? (
                  <div className="space-y-3">
                    <Link
                      href={`/grupos/${persona.grupoId}`}
                      className="block rounded-xl overflow-hidden border border-gray-100 dark:border-[#2a2a2a] bg-[#18301d]/5 dark:bg-[#0ca6b2]/10 hover:ring-2 hover:ring-[#0ca6b2]/40 transition"
                    >
                      ...
                      <div className="p-3">
                        <p className="font-semibold text-[#18301d] dark:text-white leading-snug">{persona.grupo}</p>
                        {persona.participacionEnGrupo === "apoyo" && (
                          <p className="text-xs text-violet-600 dark:text-violet-400 font-semibold mt-1">Grupo de apoyo</p>
                        )}
                        {persona.participacionEnGrupo === "colider" && (
                          <p className="text-xs text-[#0ca6b2] font-semibold mt-1">Co-líder</p>
                        )}
                        <p className="text-xs text-[#0ca6b2] font-medium mt-1">Ver grupo →</p>
                      </div>
                    </Link>
                    <button
                      type="button"
                      onClick={() => setShowParticipacionModal(true)}
                      className="w-full py-2.5 px-4 rounded-xl border border-[#0ca6b2]/30 dark:border-[#0ca6b2]/40 text-[#0ca6b2] text-sm font-semibold hover:bg-[#0ca6b2]/10 transition"
                    >
                    ...
                    <button
                      ...
                      className="w-full py-2.5 px-4 rounded-xl border border-red-200 dark:border-red-800/50 text-red-700 dark:text-red-400 text-sm font-semibold hover:bg-red-50 dark:hover:bg-red-900/20 transition"
                    >
                  ...
                  <div className="text-center py-4 space-y-3">
                    ...
                    <button
                      ...
                      className="w-full py-2.5 px-4 rounded-xl bg-[#0ca6b2] text-white text-sm font-semibold hover:bg-[#0a8f99] transition"
                    >
                      Asignar a un grupo
                    </button>
                  </div>
              ...
              <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-100 dark:border-[#2a2a2a] p-6">
                <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4">Racha de asistencia al grupo</h3>
                {ultimaAsistencia !== null ? (
                  <div className="space-y-2">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-xl bg-[#0ca6b2]/10 dark:bg-[#0ca6b2]/20 flex items-center justify-center">
                        <span className="text-xl font-bold text-[#0ca6b2]">{rachaAsistencia}</span>
                      </div>
                      <div>
                        <p className="font-semibold text-[#18301d] dark:text-white">
              ...
              <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-100 dark:border-[#2a2a2a] p-6">
                ...
                <div className="space-y-2">
                  <button 
                    onClick={() => setShowSeguimientoModal(true)}
                    className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-[#252525] transition text-left"
                  >
                    <svg className="w-5 h-5 text-[#0ca6b2] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span className="text-sm font-medium text-[#18301d] dark:text-white">Registrar seguimiento</span>
                  </button>
                  ...
                  {persona.grupoId ? (
                    <button
                      ...
                      className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-[#252525] transition text-left"
                    >
                      <svg className="w-5 h-5 text-violet-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0z" />
                      </svg>
                      <span className="text-sm font-medium text-[#18301d] dark:text-white">Co-líder / grupo de apoyo</span>
                    </button>
                  ) : null}
...
function ActionButton({ icon, color, label }: { icon: string; color: string; label: string }) {
  const colors: Record<string, string> = {
    teal: "text-[#0ca6b2]",
    coral: "text-[#e64b27]",
    yellow: "text-[#f9c70c]",
  ...
  return (
    <button className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-[#252525] transition text-left">
      <svg className={`w-5 h-5 ${colors[color]} flex-shrink-0`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        {icons[icon]}
      </svg>
      <span className="text-sm font-medium text-[#18301d] dark:text-white">{label}</span>
    </button>
  );
}
